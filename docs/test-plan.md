...
|---|----------|-------|----------|
| 1 | Empty vault | Open a brand-new vault, install plugin, run `Tag Fuzzy Find: Open` | Modal shows "No tags found in this vault." |
| 2 | Mixed inline + frontmatter | Seeded vault, `Tag Fuzzy Find: Open` | Both `#work` (inline) and `#project/work` (frontmatter) appear with correct counts, deduped |
...
| 12 | QS hook: on | Cmd+O, type `#wor` | Built-in switcher closes, Tag Fuzzy Find opens with `wor` pre-filled |
...
| 17 | Trigger prefix `:` works | Settings → Quick Switcher trigger → `:`; Cmd+O; type `:foo` | Built-in switcher closes, Tag Fuzzy Find opens with `foo` pre-filled |
| 18 | Old prefix no longer triggers | With trigger set to `:`; Cmd+O; type `#foo` | Built-in switcher behaves normally — Tag Fuzzy Find is NOT opened |
| 19 | Hook re-arms on toggle cycle | Hook OFF → change trigger to `@` → Hook ON; Cmd+O; type `@x` | Tag Fuzzy Find opens with `x`. Toggle off and `@x` should NOT trigger anymore |
